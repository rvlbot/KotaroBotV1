...
const vcard = 'BEGIN:VCARD\n' // ANAK ANJING MAU NGAPAIN ASU!?
            ...
            + 'FN:Rival Perfect\n' // NAMA OWNER BOT
            + 'ORG:Creator KOTARO BOT;\n' // KASIH CREDITS GUA RIVAL PERFECT!!
...
async function starts() {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["6285762102148@s.whatsapp.net"] // GANTI NOMOR LU
			const adminbotnumber = ["6285762102148@s.whatsapp.net"] // GANTI NOMOR LU
			const frendsowner = ["6285762102148@s.whatsapp.net"] // GAUSAH DIGANTI!!!
			...
			case 'owner':
               ...
               client.reply(from, `Waalaikumsalam ${pushname}:)`)
                ...
                case 'report':
                    ...
                    client.sendMessage('6285762102148@s.whatsapp.net', options, text, {quoted: mek})
               ...
               case 'infogempa':
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *KOTARO BROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
				...
				//frendowner
				case `addadmin`:
                   if (!isOwner) return reply('Perintah ini hanya bisa di gunakan oleh Owner Rival!', id)
                   for (let i = 0; i < mentionedJidList.length; i++) {
                   adminNumber.push(mentionedJidList[i])
                   fs.writeFileSync('./lib/database/admin.json', JSON.stringify(adminNumber))
                   reply('Success Menambahkan Admin Rival!')
				   }
                   break
				case 'wait':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
